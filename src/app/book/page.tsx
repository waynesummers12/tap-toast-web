...
function BookEventPageContent() {
  ...
  const isMountainView = venueParam === "mountain-view"

  const mountainViewPackages = {
    classic: {
      name: "Classic",
      price: 1195
    },
  ...
  const mountainViewPackage =
    isMountainView &&
    packageParam &&
    packageParam in mountainViewPackages
      ? mountainViewPackages[
...
type BookedSlot = {
  ...
  package_key: isMountainView ? packageParam : null,

  package_name: isMountainView && mountainViewPackage ? mountainViewPackage.name : null,
...
<li className={`transition-all duration-300 ${tier === "premium" ? "text-yellow-600 font-semibold" : ""} ${highlightKeys.has("setup") ? "bg-yellow-100 rounded px-1" : ""}`}>
  ✔ Extended setup time
          ...
          {isMountainView && mountainViewPackage && (
            <div className="col-span-1 md:col-span-2 mt-6 p-6 rounded-xl border border-[#c6a25a]/40 bg-[#f8f5ef] shadow-md">
              ...
              <p className="text-sm text-gray-700 mt-1">
                Your package already includes the services listed on the Mountain View Menagerie partner page.
              </p>
            </div>
          )}
...
<span className="text-xs text-gray-500 ml-2">(approximate)</span>
  ...
  {!isMountainView && <div className={`mt-10 p-8 rounded-2xl ${
    isSoda
